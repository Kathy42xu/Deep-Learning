...
###This file is predicting minority or majority.
...
def load_your_labels():
    ...
    # Convert 'spurious' column to binary labels
    labels = labels_df['spurious'].map({True: 1, False: 0}).values
    return labels

def augment_data(X, y):
    # Identify minority class
    minority_class = 1  # Adjust as necessary
    majority_class = 0  # Adjust as necessary
    ...
    num_samples = len(y_majority) - len(y_minority)
...
def main(args):
    ...
    # Load your labels
    y = load_your_labels()
    X, y, majority_class, minority_class = augment_data(X, y)
